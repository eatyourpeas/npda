...
def only_update_current_audit_year():
    """
    Can only update records for the current audit year
    """

    def check_audit_year(view, request):
        # Check requested audit year is what is currently in session
        if request.session.get("selected_audit_year") != datetime.now().year:
            logger.warning(
                f"User {request.user} tried to access {view} for audit year {request.session.get('selected_audit_year')}"
            )
            return False
        return True
    ...
    def decorator(view):
        @wraps(view)
        async def async_only_update_current_audit_year(request, *args, **kwargs):
            async_check_audit_year = sync_to_async(check_audit_year)

            if await async_check_audit_year(view, request):
                response = await view(request, *args, **kwargs)
                return response
            else:
                return redirect("npda:submissions")

        @wraps(view)
        def sync_only_update_current_audit_year(request, *args, **kwargs):
            if check_audit_year(view, request):
                return view(request, *args, **kwargs)
            else:
                return redirect("npda:submissions")

        if asyncio.iscoroutinefunction(view):
            return async_only_update_current_audit_year
        else:
            return sync_only_update_current_audit_year
